...
- Current evidence: Rust facade and core tests replay deterministic grading,
  scheduling, progression, and queue fixtures through live API surfaces. Rubric
  grading is contract-tested with a static adapter, not a model-quality corpus.
...
- Current evidence: CLI review, import probe, and web shell all keep session
  choreography outside `crates/memory-engine-core`. The web shell also records
  pressure for a compact review-state DTO and reveal semantics.
