...
| --- | --- | --- |
| Multi-consumer breadth | At least 3 independent consumers, including at least 1 non-beta-app consumer, require the same new shared ownership surface without per-client forks. | Prevents reshaping on one-workflow local pressure. |
...
| Neutral promotion candidate | A provider-neutral and persistence-neutral contract can be named with exact fields/commands, and all consumers above adopt it unchanged. | Preserves pure-kernel purity and prevents app details from leaking inward. |
...
| --- | --- | --- |
| Multi-consumer breadth | `docs/beta/multi-client-pressure.md` proves beta pressure, but no non-beta consumer requires a new shared ownership surface. | Fail |
...
| Neutral promotion candidate | `docs/beta/contract-promotion-candidate.md` identifies `ReviewStateProjection`/`ScheduleChange` as service-level shared candidate, but explicitly not ready for pure-kernel promotion. | Partial (fails trigger) |
...
- `crates/memory-engine-core`: pure kernel semantics only.
- `crates/memory-engine-service`: local command boundary (`next-queue`,
  `grade/apply-review`).
- beta boundary crates: persistence, generation, reveal UI state, and client
  choreography.
