...
Loop: /ci, /code-review, /refactor, /qa, then one final diff read. The loop exits only when `bun run ci` is green, blocking review findings are fixed, unnecessary complexity is removed, and required canary evidence is present or explicitly marked missing.
