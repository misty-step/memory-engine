...
Run or verify `bun run ci` evidence before merge. Verify any ticket-named canaries. After merge, verify active tickets were archived, then run /reflect with bounded scope: branch, merged SHA, closing IDs, and canary evidence. Harness edits from reflection go to a review branch, not directly to `master`.
